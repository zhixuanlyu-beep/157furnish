"""Editable R1 model; no rendering. Run in Blender background."""
...
for n,v in walls().items():cube(n,v['box'],v['z'],v['height'],'warmwhite',arch)
...
for n,v in D['furniture'].items():
 ...
 if kind=='cabinet':
  ...
  if kind=='chair':part('back',[x,y,w,35],z+450,h-450,'green')
 elif kind=='sofa':
  ...
  b=[x,y,20,d] if front=='west' else [x+w-20,y,20,d] if front=='east' else [x,y,w,20]
...
for n,b in D['states']['six_extra'].items():cube(n,b,0,800,'green',six)
for n,pos,target in [('Whole',(20,-16,19),(7.5,5,0)),('Living',(14,3,2.2),(10.5,7,1)),('Kitchen',(13,4.7,2.2),(14,7,1)),('Master',(4,1.7,2.2),(2,3.2,.8)),('Child',(5.6,5,2.2),(2.7,6.8,.8)),('Guest',(8.5,5,2.2),(7.4,7,.8)),('Top',(7.5,5,22),(7.5,5,0))]:
...
for n,loc in [('Ceiling',(8,5,2.6)),('Window',(12,9,3))]:
...
dump('model/projection_snapshot.json',bound({'generator_sha256':hashfile(__file__),'furniture':snapshot,'state':'frame1'}))
bpy.ops.wm.save_as_mainfile(filepath=str(ROOT/'model/157furnish_R1.blend'))
bpy.ops.object.select_all(action='DESELECT')
for c in [arch,furn,doors]:
 for o in c.objects:o.select_set(True)
bpy.ops.export_scene.gltf(filepath=str(ROOT/'model/157furnish_R1.glb'),export_format='GLB',use_selection=True,export_animations=False,export_extras=True)
dump('model/scene_config.json',bound({'units':'metres','furniture':{n:{'box_m':[x/1000 for x in v['box']],'height_m':v['height']/1000,'z_m':v['z']/1000} for n,v in D['furniture'].items()},'blender':bpy.app.version_string,'cameras':7,'rendered':False}))
