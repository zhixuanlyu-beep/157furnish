...
def main():
 ...
 for i in range(2 if a.repeat else 1):
  for script in ['verify_usage.py','build_model.py','verify_model.py','build_drawings.py','publish_docs.py']:
   ...
   result=subprocess.run(args,cwd=R,stdout=subprocess.PIPE,stderr=subprocess.STDOUT);(R/'reports'/(script[:-3]+'.log')).write_bytes(result.stdout)
  ...
  subprocess.run([sys.executable,'scripts/validate_delivery.py'],cwd=R,check=True,stdout=subprocess.PIPE)
