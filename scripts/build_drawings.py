...
class Sheet:
 def __init__(self,title,notes=()):
  self.title=title;self.svg=[f'<svg xmlns="http://www.w3.org/2000/svg" width="1800" height="1250" viewBox="0 0 1800 1250" data-revision="{REV}" data-layout-sha256="{SHA}">'];self.im=Image.new('RGB',(W,H),'#fcfbf6');self.dr=ImageDraw.Draw(self.im);self.rect([0,0,W,H],'#fcfbf6');self.text(45,26,'157FURNISH  /  R1',22,'#576858');self.text(45,66,title,34);self.line([(45,119),(1755,119)],'#456653',2)
 ...
 def plan(self,data=D,crop=None,ops=False,route=None,six=False,baseline=False,services=False,hvac=False,alter=False,labels=True,opened=False):
  ...
  for n,o in openings(data).items():
   ...
   elif o['kind']=='sliding':self.rect(rb(slide_box(o,opened)),'#aac9c9','#466b74')
  if not baseline:
  ...
  if alter:
   e=next(e for e in D['edges'] if e['id']=='kitchen_w');self.line([self.pt(*e['a']),self.pt(*e['b'])],'#ca5944',7)
  if route:
   for rr in route:
...
def solar():
 ...
 return rows
def main():
 snap=json.loads((ROOT/'model/projection_snapshot.json').read_text(encoding='utf-8'));assert snap['layout_sha256']==SHA,'拒绝过期模型投影'
 assert snap['generator_sha256']==hashfile(ROOT/'scripts/build_model.py'),'模型脚本变更须重建模型'
 ...
 q=Sheet('01 原始尺寸基线 / 尺寸链与假设分离',D['source_notes']+['黑墙为概念重建；原照片作为独立原始凭证。','总宽15077，总深10074。']);q.plan(baseline=True)
 ...
 q=Sheet('02 推荐家具总图 / 三房两卫', ['B主卧、A儿童、C客房。','推荐厨房可关闭、岛台无水。','总图用对象编号，逐室图标中文名称及尺寸。','所有门关闭；开启、路线另图。','两处淋浴保留在原湿区。']);q.plan();q.save('02-plan.svg')
 for state,title in [('normal4','四人正常就座'),('normal6','六人增加端椅'),('chairs_pulled','餐椅拉出450状态'),('laundry_open','洗烘开启/装卸状态')]:
  q=Sheet('使用情景 / '+title,['蓝线为620mm方篮可达路线。','棕框为使用包络；不代表同时可用。','餐桌1600×850；六人端位需试坐。','所有通行门按完全开启。','查看路线表中的失败记录，图中不画虚假通过线。']);q.plan(ops=state in ['chairs_pulled','laundry_open'],six=state=='normal6',opened=True,route=[r for r in U['routes'] if r['state']==state and r['width_mm']==620 and r['name']=='洗烘']);q.save('03-'+state+'.svg')
 ...
 q=Sheet('07 局部拆改条件 / 厨房西侧界面',['红色仅表示开放候选界面。','推荐保持原分隔，不对墙体作结构承诺。','原门洞位置/宽度均待测；推拉挂轨及封闭性待核。','岛槽排水线路未落实，推荐干岛。','两卫不扩湿区，需核地漏、坑距及防水。']);q.plan(alter=True);q.save('07-alterations.svg')
 for n,r in D['rooms'].items():
  ...
  q=Sheet('逐室 / '+r['label'],notes[:12]);q.plan(crop=[x,y,xx-x,yy-y],opened=True);q.save('08-room-'+n+'.svg')
 ...
 q=Sheet('11 水电概念点位',['红点S编号对应右侧清单及CSV。','燃气与主槽保留原区域；接口待测。','地插、防溅、防冻与回路由专业深化。']);q.plan(services=True,labels=False)
 ...
 page='''<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>157furnish R1 离线方案册</title><style>body{background:#f5f2e8;color:#273e34;font:17px/1.7 "Microsoft YaHei",sans-serif;margin:0}main{max-width:1400px;margin:auto;padding:30px}header{background:#294a3a;color:#fff;padding:35px}a{color:#366e51}figure{background:white;margin:25px 0;padding:16px}img{width:100%;height:auto}aside{background:#eadfc9;padding:24px;margin:20px 0}details{padding:16px;background:#fff}li{margin:5px} @media print{details{display:none}figure{break-inside:avoid}main{padding:0}} </style><main><header><h1>157furnish · R1</h1><p>三房两卫 · 可关闭厨房 · 无水岛台 · 两处淋浴</p><p>本地可编辑图模交付；没有生成三维效果图。</p></header><aside>全部尺寸为原图标注、推导或显式概念参数。固定实体及500单人基线通过不等于全部使用通过：携篮、900目标、设备开启及多人操作存在限制。请先读核验摘要和现场条件。</aside><p><a href="verification.md">核验摘要</a> · <a href="constraints.md">现场条件</a> · <a href="design.md">设计取舍</a> · <a href="handoff.md">接手重建</a> · <a href="../model/157furnish_R1.blend">Blender</a> · <a href="../model/157furnish_R1.glb">GLB</a> · <a href="../IMG20260921-135355869.jpg">原始户型图</a></p><details><summary>图纸目录</summary><ul>'''+navigation+'</ul></details>'+figures+'<h2>表格</h2><ul>'+links+'</ul><p>R1 / SHA256 '+SHA+'</p></main></html>'
